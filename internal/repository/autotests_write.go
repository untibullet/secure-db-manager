...
func (r *Repository) CreateAutotest(ctx context.Context, dto AutotestDTO) (int, error) {
	sql := `INSERT INTO v_engineer_autotests (name, description, owner_user_id, is_active) 
            VALUES ($1, $2, $3, $4) RETURNING autotest_id`
	var id int
	err := r.db.QueryRow(ctx, sql, dto.Name, dto.Description, dto.OwnerID, dto.IsActive).Scan(&id)
	return id, err
}

func (r *Repository) UpdateAutotest(ctx context.Context, id int, dto AutotestDTO) error {
    sql := `
        UPDATE v_engineer_autotests 
        SET name = $1, 
            description = $2, 
            is_active = $3
        WHERE autotest_id = $4
...
func (r *Repository) DeleteAutotest(ctx context.Context, id int) error {
    sql := `DELETE FROM v_engineer_autotests WHERE autotest_id = $1`
