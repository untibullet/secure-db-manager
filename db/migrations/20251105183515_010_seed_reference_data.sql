...
INSERT INTO roles (code, name, description, db_role_name, access_level) VALUES
    ('ADMIN',               'Администратор',            'Полный доступ к системе, управление пользователями и ролями', 'db_admin',               10),
    ('TEST_LEAD',           'Тест-лид',                 'Управление тест-планами, кейсами и командой тестирования',    'db_test_lead',            8),
    ('AUTOMATION_ENGINEER', 'Инженер по автоматизации', 'Разработка и поддержка автотестов',                          'db_automation_engineer',  7),
    ('TESTER',              'Тестировщик',              'Выполнение тестов и создание тест-кейсов',                    'db_tester',               5),
    ('CI_CD_SYSTEM',        'CI/CD система',            'Автоматическое выполнение тестов через CI/CD pipeline',       'db_cicd_system',          6),
    ('DEVELOPER',           'Разработчик',              'Просмотр результатов и создание конфигураций',                'db_developer',            4),
    ('GUEST',               'Гость',                    'Только просмотр публичных данных',                           'db_guest',                1);
-- +goose StatementEnd
